...
PUBLIC namespace {
    ...
    PRIVATE auto idle( IN Kernel::Task::ProcessManagement::TaskArg )->Kernel::Task::ProcessManagement::TaskReturnValue {
        while ( TRUE ) {
            Kernel::Task::ThreadManagement::block( Kernel::Task::ThreadManagement::TaskStatus::BLOCKED );
            Architecture::ArchitectureManagement< TARGET_ARCH >::sti( );
...
PUBLIC namespace QuantumNEC::Kernel::Task {
    ProcessManagement::ProcessManagement( VOID ) noexcept {
        Lib::STL::list_init( &this->all_task_queue );
        Lib::STL::list_init( &this->ready_task_queue );
        // 准备内核进程PCB, IDLE PCB
        this->kernel_task = get_current< ProcessPCB >( );
        this->main_task = reinterpret_cast< decltype( this->main_task ) >( Architecture::ArchitectureManagement< TARGET_ARCH >::kernel_stack_space );
        ...
        this->main_task->fpu_frame = reinterpret_cast< Lib::Types::Ptr< Architecture::ArchitectureManagement< TARGET_ARCH >::FPUFrame > >( this->main_task->cpu_frame + 1 );
    ...
    auto ProcessManagement::create( IN TaskFunction entry, IN CONST TaskArg arg, IN CONST TaskType type, IN IN CONST Lib::Types::Ptr< CONST Lib::Types::char_t > name, IN Lib::Types::uint64_t priority, IN CONST Lib::Types::int64_t flags )->Lib::Types::Ptr< ProcessPCB > {
        lock.acquire( );
        Lib::Types::Ptr< ProcessPCB > task { reinterpret_cast< decltype( task ) >( Memory::PageMemoryManagement::malloc( 1, Memory::PageMemoryManagement::MemoryPageType::PAGE_2M ) ) };
        if ( !task ) {
            return NULL;
        }
        // 将任务pcb添加到队列
        ...
        if ( !task->page_directory ) {
            ...
            Memory::PageMemoryManagement::free( task, 1, Memory::PageMemoryManagement::MemoryPageType::PAGE_2M );
            return NULL;
        }
        ...
        // 分配 fpu frame
        task->fpu_frame = reinterpret_cast< Lib::Types::Ptr< Architecture::ArchitectureManagement< TARGET_ARCH >::FPUFrame > >( task->cpu_frame + 1 );
    ...
    auto ProcessManagement::do_fork( OUT Lib::Types::Ptr< ProcessFrame > frame, IN TaskFunction entry, IN CONST TaskArg arg, IN CONST TaskType type )->Lib::Types::BOOL {
        ...
        if ( type == TaskType::PF_KERNEL_PROCESS ) {
            // 内核进程的
            frame->regs.ds = Architecture::Platform::SELECTOR_DATA64_KERNEL;
            frame->regs.es = Architecture::Platform::SELECTOR_DATA64_KERNEL;
            frame->regs.fs = Architecture::Platform::SELECTOR_DATA64_KERNEL;
            frame->regs.gs = Architecture::Platform::SELECTOR_DATA64_KERNEL;
            frame->cs = Architecture::Platform::SELECTOR_CODE64_KERNEL;
            frame->ss = Architecture::Platform::SELECTOR_DATA64_KERNEL;
            frame->rflags = Architecture::Platform::RFLAGS_IOPL_3 | Architecture::Platform::RFLAGS_MBS | Architecture::Platform::RFLAGS_IF_1;
        }
        else if ( type == TaskType::PF_USER_PROCESS ) {
            // 用户进程的
            frame->regs.ds = Architecture::Platform::SELECTOR_DATA64_USER;
            frame->regs.es = Architecture::Platform::SELECTOR_DATA64_USER;
            frame->regs.fs = Architecture::Platform::SELECTOR_DATA64_USER;
            frame->regs.gs = Architecture::Platform::SELECTOR_DATA64_USER;
    ...
    auto ProcessManagement::schedule( VOID )->VOID {
        ...
        if ( Lib::STL::list_is_empty( &ready_task_queue ) ) {
            ThreadManagement::unblock( ThreadManagement::idle_task );
        }
        ...
        Architecture::ArchitectureManagement< TARGET_ARCH >::disable_fpu( current );     // 关闭FPU
